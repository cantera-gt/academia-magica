...
export default function SubjectOrderCheckout({
  orderId,
  total,
  currency,
  paid,
  ...
  async function startPayment() {
    ...
    setError("");
    try {
      const response = await fetch("/api/paypal/subjects/create-order", {
        method: "POST",
  ...
  if (paid) {
    return (
      <div className="rounded-2xl bg-emerald-50 p-5 font-extrabold text-emerald-800">
  ...
  return (
    <>
      ...
      <p className="mt-3 text-sm text-[#3b2a55]/60">
        Serás enviado al entorno seguro Sandbox de PayPal. No se utilizará dinero real durante las
        pruebas.
      </p>
      {error && (
        <p role="alert" className="mt-4 rounded-2xl bg-red-50 p-4 font-bold text-red-700">
          {error}
        </p>
      )}
    </>
