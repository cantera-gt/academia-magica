...
export default function AlumnosPage() {
  ...
  const visibleStudents = useMemo(() => {
    ...
    return students
      .filter((student) => {
        if (normalized && !`${student.display_name} ${student.username ?? ""} ${student.household_name} ${student.guardian_name ?? ""} ${student.guardian_email ?? ""} ${student.tags.join(" ")}`.toLocaleLowerCase("es").includes(normalized)) return false;
              ...
              <article key={student.student_id} className={`p-4 ${!student.is_active ? "bg-slate-50" : ""}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2"><div><Link href={`/admin/alumnos/${student.student_id}`} className="font-extrabold text-slate-900">{student.display_name}</Link><p className="text-xs text-slate-400">@{student.username ?? "sin usuario"} · {student.household_name}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-extrabold ${student.is_active ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-600"}`}>{student.is_active ? "Activo" : "Inactivo"}</span></div>
